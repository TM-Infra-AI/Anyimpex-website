...
class Woo_Twitter extends WP_Widget {
    ...
    public function __construct() {
        ...
        parent::__construct('woo_twitter', __('Woo - Twitter Stream', 'woothemes'), $widget_ops);
    }
    ...
    function widget($args, $instance) {
        ...
        if ($title) {
            ...
            ?>
            <h3 class="tlogo"><img src="<?php bloginfo('template_directory'); ?>/images/twitter.png" alt="Twitter" /><?php _e(' Twitter','woothemes'); ?></h3>
            <?php
        }
        ?>
        <div class="back">
            <ul id="twitter_update_list_<?php echo $unique_id; ?>"><li></li></ul>
            <p><?php _e('Follow','woothemes'); ?> <a href="http<?php (is_ssl() ? 's' : '') ?>://twitter.com/<?php echo esc_html($username); ?>"><strong>@<?php echo esc_html($username); ?></strong></a> <?php _e('on Twitter','woothemes'); ?></p>
        </div>
        <div class="clear"></div>
        <?php
    ...
    function woo_enforce_defaults($instance) {
        ...
        $instance['title'] = strip_tags($instance['title']); // Not for security so much as to give feedback that HTML isn't allowed
...
// Register the widget
function register_woo_twitter_widget() {
    register_widget('Woo_Twitter');
}
add_action('widgets_init', 'register_woo_twitter_widget');
